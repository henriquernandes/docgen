...
import {RiArrowDropDownLine, CgTrash, BsPencil} from "react-icons/all";
...
const DropdownList = ({ teamName }) => {
    const [isOpen, setIsOpen] = useState(false);
    const users = ["Usuario","Usuario","Usuario"]
    return(
        <>
            <div className="text-white bg-gray-600 w-full py-4 mt-4 rounded flex flex-row items-center justify-between"
                 onClick={() => setIsOpen(prev => !prev)}
            >
                <h2 className="pl-3">{teamName}</h2>
                <RiArrowDropDownLine className="mr-3" size="2em" />
            </div>
            {isOpen &&
                <div className="text-white bg-gray-600 w-full py-4 -mt-2"
                >
